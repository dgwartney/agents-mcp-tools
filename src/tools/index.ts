...
import { sendMessage, sendMessageSchema, resetSession, resetSessionSchema } from './interaction.js';
import {
  listActiveSessions,
...
export const tools: ToolDefinition[] = [
  ...
  {
    name: 'debug_reset_session',
    description: 'Reset the current debug session. Clears conversation history and trace events.',
    schema: resetSessionSchema,
    handler: resetSession as (args: unknown, ctx: DebugContext) => Promise<string>,
  },
...
Requires HARNESS_API_KEY environment variable.`,
  ...
  // Platform management tools
  {
    ...
    description:
      'Manage projects on the platform. Actions: list (all projects), get (by projectId), create (with name/description), delete (by projectId).',
  ...
  },
  {
    ...
    description:
      'Import and export projects. Actions: export_preview (metadata preview), export (full project export as file map + manifest), import_preview (dry-run import showing changes), import (apply import).',
