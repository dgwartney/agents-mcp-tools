...
describe('connect tool', () => {
  ...
  describe('remote URL — skips health check', () => {
    test('skips health check for remote URL and goes straight to auth + WS', async () => {
      const ctx = createMockContext();
      vi.mocked(ctx.httpClient.getBaseUrl).mockReturnValue('https://abl-dev.kore.ai');
      vi.mocked(ctx.wsClient.getUrl).mockReturnValue('wss://abl-dev.kore.ai/ws');

      const raw = await connect({ serverUrl: 'https://abl-dev.kore.ai' }, ctx);
      const result = JSON.parse(raw);
    ...
    test('returns WS error with details when remote WS connection fails', async () => {
      ...
      const wsError = new Error(
        'WebSocket connection timed out after 10s connecting to wss://abl-dev.kore.ai/ws',
      );
